...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.eventAttendance.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.event-attendances.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('dates') ? 'is-invalid' : '' }}" type="text" name="dates" id="dates" value="{{ old('dates', '') }}">
                @if($errors->has('dates'))
                    <div class="invalid-feedback">
                        {{ $errors->first('dates') }}
                    </div>
