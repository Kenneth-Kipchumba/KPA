...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.courses.update", [$course->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="specialization_id">{{ trans('cruds.course.fields.specialization') }}</label>
                <select class="form-control select2 {{ $errors->has('specialization') ? 'is-invalid' : '' }}" name="specialization_id" id="specialization_id">
                    @foreach($specializations as $id => $specialization)
                        <option value="{{ $id }}" {{ (old('specialization_id') ? old('specialization_id') : $course->specialization->id ?? '') == $id ? 'selected' : '' }}>{{ $specialization }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.course.fields.specialization_helper') }}</span>
            </div>
            <div class="form-group">
