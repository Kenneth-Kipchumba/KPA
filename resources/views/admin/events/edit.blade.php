...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.events.update", [$event->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="specialization_id">{{ trans('cruds.event.fields.specialization') }}</label>
                <select class="form-control select2 {{ $errors->has('specialization') ? 'is-invalid' : '' }}" name="specialization_id" id="specialization_id">
                    @foreach($specializations as $id => $specialization)
                        <option value="{{ $id }}" {{ (old('specialization_id') ? old('specialization_id') : $event->specialization->id ?? '') == $id ? 'selected' : '' }}>{{ $specialization }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.event.fields.specialization_helper') }}</span>
            </div>
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('category') ? 'is-invalid' : '' }}" name="category" id="category">
                    ...
                    @foreach(App\Models\Event::CATEGORY_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('category', $event->category) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label for="dates">{{ trans('cruds.event.fields.dates') }}</label>
                <input class="form-control {{ $errors->has('dates') ? 'is-invalid' : '' }}" type="text" name="dates" id="dates" value="{{ old('dates', $event->dates) }}">
                @if($errors->has('dates'))
                    <div class="invalid-feedback">
                        {{ $errors->first('dates') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.event.fields.dates_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('start_date') ? 'is-invalid' : '' }}" type="text" name="start_date" id="start_date" value="{{ old('start_date', $event->start_date) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('end_date') ? 'is-invalid' : '' }}" type="text" name="end_date" id="end_date" value="{{ old('end_date', $event->end_date) }}">
